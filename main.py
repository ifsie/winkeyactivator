...
def get_windows_dir():
    try:
        ...
        print("\nerror: sys root environment variable not set")
        exit(1)
...
def read_activation_keys(): # gather keys from activation_keys.txt
    try:
        ...
        print("\nerror: activation_keys.txt file not found")
        exit(1)

def activate_windows(windows_dir, activation_key):
    try:
...
def remove_key_from_file(activation_keys, activation_key):
    try:
...
def archive_key(activation_key):
    try:
...
def main():
    ...
    if not check_activation_status(windows_dir):
